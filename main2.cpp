...
void add_limit_order(order& ord_er){
	if(ord_er.quantity<=0||ord_er.price<=0){
		cout<<"INVALID ORDER either because of a invalid price or quantity\n";
	}
	else if(ord_er.side==orderside::BUY){
		buy[ord_er.price].push_back(ord_er);
		cout<<"Order with order id->"<<ord_er.id<<" Price->"<<ord_er.price<<" and quantity->"<<ord_er.quantity<<" ADDED TO BUY\n";
	}
	else{
		sell[ord_er.price].push_back(ord_er);
		cout<<"Order with order id->"<<ord_er.id<<" Price->"<<ord_er.price<<" and quantity->"<<ord_er.quantity<<" ADDED TO SELL\n";
	}
...
void match(order& ord_er){
	if(ord_er.side==orderside::BUY){
		...
		while(sell_pr.size()>0){
			if(ord_er.quantity<=0){cout<<"ORDER QUANTITIY IS ZERO.";break;}
			else if(sell_pr.begin()->quantity>ord_er.quantity){
				cout<<"Order id "<<ord_er.id<<" and quantity->"<<ord_er.quantity<<endl;
				cout<<"SELL_BOOK_ price="<<sell_list->first<<" ord_er id="<<sell_pr.begin()->id<<" quantity="<<sell_pr.begin()->quantity<<"\n";
				sell_pr.begin()->quantity-=ord_er.quantity;
				ord_er.quantity=0;
			}
			else{
				cout<<"Order id "<<ord_er.id<<" and quantity->"<<ord_er.quantity<<endl;
				cout<<"SELL_BOOK_ price="<<sell_list->first<<" order id="<<sell_pr.begin()->id<<" quantity="<<sell_pr.begin()->quantity<<"\n";
		...
		while(buy_pr.size()>0){
			if(ord_er.quantity<=0){cout<<"ORDER QUANTITIY IS ZERO.";break;}
				...
				cout<<"Order id "<<ord_er.id<<" and quantity->"<<ord_er.quantity<<endl;
				cout<<"BUY_BOOK_ price="<<buy_list->first<<" order id="<<buy_pr.begin()->id<<" quantity="<<buy_pr.begin()->quantity<<"\n";
	...
	else{cout<<"ORDER NOT MATCHED.\n";add_limit_order(ord_er);}
...
void cancel_order(int order_id){
	if(cancel_map.find(order_id)!=cancel_map.end()){
		...
		if(sid==orderside::BUY){
			...
			while(it!=buy[pri].end()){
				if(it->id==order_id){
					cout<<"Order id->"<<order_id<<" is deleted from the price level="<<pri<<" of buy order\n";
					it=buy[pri].erase(it);
			...
			while(it!=sell[pri].end()){
				if(it->id==order_id){
					cout<<"Order id->"<<order_id<<" is deleted from the price level="<<pri<<" of sell order\n";
	...
	else{cout<<"ORDER ID NOT FOUND.\n";}
}

void test_case_0() {
    order buy_order = {1, 100, 0, orderside::BUY}; // Order ID 1, Price 100.0, Quantity 10, Side BUY
    order sell_order = {2, 100, 10, orderside::SELL}; // Order ID 2, Price 100.0, Quantity 10, Side SELL
...
void test_case_1() {
    order buy_order = {1, 100, 10, orderside::BUY}; // Order ID 1, Price 100.0, Quantity 10, Side BUY
    order sell_order = {2, 100, 10, orderside::SELL}; // Order ID 2, Price 100.0, Quantity 10, Side SELL
    ...
    show();
    std::cout << "\n--- Matching BUY Order ---\n";
    match(*sell[sell_order.price].begin());
}

void test_case_2() {
    order sell_order = {1, 101, 15, orderside::SELL}; // SELL 15 units at Price 101.0
    order buy_order = {2, 101, 10, orderside::BUY};   // BUY 10 units at Price 101.0
    ...
    match(*buy[buy_order.price].begin());
}

void test_case_3() {
    order sell_order1 = {1, 100, 5, orderside::SELL};
    order sell_order2 = {2, 101, 10, orderside::SELL};
    order buy_order = {3, 101, 15, orderside::BUY};
    show();
    add_limit_order(sell_order1);
    add_limit_order(sell_order2);
    add_limit_order(buy_order);
...
void test_case_4() {
    order sell_order = {1, 100, 10, orderside::SELL}; // SELL Order ID 1
    add_limit_order(sell_order);
    
    std::cout << "\n--- Canceling Order ---\n";
    cancel_order(1); // Canceling Order ID 1
}

void test_case_5() {
    order sell_order1 = {1, 100, 5, orderside::SELL};
    order sell_order2 = {2, 101, 10, orderside::SELL};
    order sell_order3 = {3, 102, 10, orderside::SELL};
    order buy_order = {4, 103, 20, orderside::BUY}; // Matches 15 units from two levels and leaves 5 units in SELL
    
    add_limit_order(sell_order1);
    add_limit_order(sell_order2);
    add_limit_order(sell_order3);
...
void test_case_6() {
    order buy_order = {1, 100, 10, orderside::BUY};
    
    std::cout << "\n--- Matching BUY Order (Empty SELL Book) ---\n";
    match(buy_order);
    
    // Ensure the unmatched order is added as a new BUY limit order.
}

void simulate_order_book(int num_orders) {
    srand(50);  // Seed for reproducibility
    ...
    for (int i = 0; i < num_orders; ++i) {
        ...
        if (action < 40) {
            ...
            if (!buy.empty()) {
                int64_t price = 11000.0;  // Low price to ensure matching
                double quantity = 1 + rand() % 50;
